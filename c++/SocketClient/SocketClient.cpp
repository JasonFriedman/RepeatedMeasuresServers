#define WIN32_LEAN_AND_MEAN

// Based on https://docs.microsoft.com/en-us/windows/win32/winsock/complete-client-code

#include <windows.h>
#include <winsock2.h>
#include <ws2tcpip.h>
#include <stdlib.h>
#include <stdio.h>
#include "messagecodes.h"
#include "SocketClient.h"

// Need to link with Ws2_32.lib, Mswsock.lib, and Advapi32.lib
#pragma comment (lib, "Ws2_32.lib")
#pragma comment (lib, "Mswsock.lib")
#pragma comment (lib, "AdvApi32.lib")

#define DEFAULT_BUFLEN 512

SOCKET connectToSocket(char* server,char* port) {

	WSADATA wsaData;
	SOCKET ConnectSocket = INVALID_SOCKET;
	struct addrinfo *result = NULL,
		*ptr = NULL,
		hints;

	char recvbuf[DEFAULT_BUFLEN];
	int iResult;
	int recvbuflen = DEFAULT_BUFLEN;

	// Initialize Winsock
	iResult = WSAStartup(MAKEWORD(2,2), &wsaData);
	if (iResult != 0) {
		printf("WSAStartup failed with error: %d\n", iResult);
		return 1;
	}

	ZeroMemory( &hints, sizeof(hints) );
	hints.ai_family = AF_UNSPEC;
	hints.ai_socktype = SOCK_STREAM;
	hints.ai_protocol = IPPROTO_TCP;

	// Resolve the server address and port
	iResult = getaddrinfo(server, port, &hints, &result);
	if ( iResult != 0 ) {
		printf("getaddrinfo failed with error: %d\n", iResult);
		WSACleanup();
		return 1;
	}

	// Attempt to connect to an address until one succeeds
	for(ptr=result; ptr != NULL ;ptr=ptr->ai_next) {

		// Create a SOCKET for connecting to server
		ConnectSocket = socket(ptr->ai_family, ptr->ai_socktype, 
			ptr->ai_protocol);
		if (ConnectSocket == INVALID_SOCKET) {
			printf("socket failed with error: %ld\n", WSAGetLastError());
			WSACleanup();
			return 1;
		}

		// Connect to server.
		iResult = connect( ConnectSocket, ptr->ai_addr, (int)ptr->ai_addrlen);
		if (iResult == SOCKET_ERROR) {
			closesocket(ConnectSocket);
			ConnectSocket = INVALID_SOCKET;
			continue;
		}
		break;
	}

	freeaddrinfo(result);

	if (ConnectSocket == INVALID_SOCKET) {
		printf("Unable to connect to server!\n");
		WSACleanup();
		return 1;
	}

	// Wait for acknowledgement (the letter R) - this is blocking
	iResult = recv(ConnectSocket, recvbuf, recvbuflen, 0);
	printf("Received acknowledgement %s\n",recvbuf);
	if (recvbuf[0] != 'R') {
		printf("Did not receive appropriate acknowledgement from server (R)\n");
		closesocket(ConnectSocket);
		WSACleanup();
		return 1;
	}

	return ConnectSocket;
}

int sendMessage(SOCKET ConnectSocket, char* sendbuf) {

	int iResult;

	// Send a message
	iResult = send( ConnectSocket, sendbuf, (int)strlen(sendbuf), 0 );
	if (iResult == SOCKET_ERROR) {
		printf("send failed with error: %d\n", WSAGetLastError());
		closesocket(ConnectSocket);
		WSACleanup();
		return 1;
	}

	//printf("Bytes Sent: %ld\n", iResult);
	return iResult;

}

int receiveMessage(SOCKET ConnectSocket, char* recvbuf, int recvbuflen) {
	int iResult = recv(ConnectSocket, recvbuf, recvbuflen, 0);
	if ( iResult > 0 )
		;//printf("Bytes received: %d\n", iResult);
	else if ( iResult == 0 )
		printf("Connection closed\n");
	else
		printf("recv failed with error: %d\n", WSAGetLastError());

	return iResult;
}

int closeSocket(SOCKET ConnectSocket) {

	int iResult;

	// shutdown the connection since no more data will be sent
	iResult = shutdown(ConnectSocket, SD_SEND);
	if (iResult == SOCKET_ERROR) {
		printf("shutdown failed with error: %d\n", WSAGetLastError());
		closesocket(ConnectSocket);
		WSACleanup();
		return 1;
	}

	// cleanup
	closesocket(ConnectSocket);
	WSACleanup();

	return 0;
}

void setupRecording(SOCKET ConnectSocket, char* filename, int nummarkers, int maxtime) {

	char message[50];
	sprintf(message,"%c,%s,%d,%d",SETUPRECORDING,filename,nummarkers,maxtime);
	printf("Sending message: %s\n",message);
	sendMessage(ConnectSocket,message);
}

void startRecording(SOCKET ConnectSocket) {
	char message[50];
	sprintf(message,"%c",STARTRECORDING);
	printf("Sending message: %s\n",message);
	sendMessage(ConnectSocket,message);
}

void stopRecording(SOCKET ConnectSocket) {
	char message[50];
	sprintf(message,"%c",STOPRECORDING);
	printf("Sending message: %s\n",message);
	sendMessage(ConnectSocket,message);
}

void saveFile(SOCKET ConnectSocket) {
	char message[50];
	sprintf(message,"%c",SAVEFILE);
	printf("Sending message: %s\n",message);
	sendMessage(ConnectSocket,message);
}

void closeDevice(SOCKET ConnectSocket) {
	char message[50];
	sprintf(message,"%c",CLOSEDEVICE);
	printf("Sending message: %s\n",message);
	sendMessage(ConnectSocket,message);
}

void markEvent(SOCKET ConnectSocket, int eventNumber) {
	char message[50];
	sprintf(message,"%c,%d",MARKEVENT,eventNumber);
	printf("Sending message: %s\n",message);
	sendMessage(ConnectSocket,message);
}